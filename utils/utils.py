...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    try:
        ...
        print('y' + str(int(x[1])))
        if(int(x[1]) >= 965 and int(x[1]) <= 1500):
            crop_img = imcpy[int(x[1])-10:(int(x[3])+10),int(x[0])-10:int(x[2])+10]
            cv2.imshow("Hasil Plate", crop_img)
            if cv2.waitKey(1) == ord('q'):  # q to quit
                raise StopIteration
            digitsRec = reader.readtext(crop_img, detail = 0, allowlist = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ',width_ths = 1.5, text_threshold=0.8)
            digitsRec.reverse()
            plate.join(digitsRec)
            print(digitsRec)
            plate = listToString(digitsRec)
        else:
            plate = 'Plate'
        if(plate == ''):plate = 'Plate'
        if label:
            tf = max(tl - 1, 1)  # font thickness
            t_size = cv2.getTextSize(plate, 0, fontScale=tl / 3, thickness=tf)[0]
            c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
            cv2.rectangle(img, c1, c2, (255,0,0), -1, cv2.LINE_AA)  # filled
            cv2.putText(img,plate , (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
